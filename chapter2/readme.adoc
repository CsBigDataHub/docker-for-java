...
docker container run -d --name web -p 8080:8080 jboss/wildfly #this command will map the ports
docker container logs web
```
...
. Explain `COPY` vs `ADD`
-> COPY - Copy new files or directories to the container filesystem
-> ADD - has all the features of COPY and also allows tar file autoextraction in the image
            -eg: ADD app.tar.gz /opt/var/myapp
        Can also download files from a remote url
...
-> RUN - used for installing software package
        eg: RUN apt-get update && apt-get install -y git
